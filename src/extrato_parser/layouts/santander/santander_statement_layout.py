...
class SantanderStatementLayout(StatementLayout):
    def matches(self, lines: list[PhysicalLine]) -> bool:
        sample = plain_text(" ".join(line.text for line in lines[:60]))
        return "SANTANDER" in sample or (
            "DATA" in sample and "DOCUMENTO" in sample and "MOVIMENT" in sample and "SALDO" in sample
        )
